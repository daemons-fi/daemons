...
describe('GET api/auth/message-to-sign/:userAddress', () => {
    ...
    it('retrieved message includes user address with checksum', async () => {
        const response = await supertest(app)
            .get(`/api/auth/message-to-sign/${userAddress}`)
            .expect(200);

        const checksumUserAddress = utils.getAddress(userAddress);
        const message = response.body.message;
        expect(message).to.include(checksumUserAddress);
    });
